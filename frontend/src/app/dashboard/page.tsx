...
export default function DashboardPage() {
  ...
  const kpis = [
    {
      label: 'Revenue Today',
      value: tickets ? formatKES(tickets.revenueToday) : '--',
      sub: tickets ? `${formatKES(tickets.totalRevenue)} total` : '',
      icon: (
    ...
    {
      label: 'Tickets Today',
      value: tickets ? String(tickets.ticketsToday) : '--',
      sub: tickets ? `${tickets.totalTickets} total` : '',
      icon: (
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    },
    {
      ...
      sub: fleet ? `${fleet.totalTrips} total` : '',
      icon: (
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  const recentActivities = [
    {
      ...
      description: 'Nairobi → Mombasa, KES 1,500',
      time: '2 min ago',
      color: 'bg-primary/10 text-primary',
    },
    {
      title: 'Trip completed',
      description: 'Bus KAV-4521 arrived at Kisumu terminal',
      time: '8 min ago',
      color: 'bg-emerald-50 text-emerald-600',
    },
    {
      title: 'Parcel delivered',
      description: 'PKG-8834 collected in Nakuru',
      time: '15 min ago',
      color: 'bg-amber-50 text-amber-600',
    },
    {
      title: 'Staff check-in',
      description: 'John Kamau clocked in at Nairobi depot',
      time: '22 min ago',
      color: 'bg-sky-50 text-sky-600',
    },
    {
      title: 'Fuel expense logged',
      description: 'KES 12,400 for Bus KAV-1098',
      time: '35 min ago',
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold tabular-nums text-emerald-600">
                  {employees?.compliant ?? 0}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground">Expiring Docs</span>
                <span className="text-sm font-semibold tabular-nums text-amber-600">
                  {employees?.expiringDocuments ?? 0}
                </span>
              </div>
              <div className="flex items-center justify-between border-t border-border pt-2">
